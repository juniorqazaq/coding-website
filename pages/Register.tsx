...
import { Eye, EyeOff, Check, ArrowRight, Lock, Mail, User, Code2 } from 'lucide-react';
            ...
            <div className="t-auth-wrap page-enter">
                ...
                <div className="t-auth-card">
                    <div className="t-auth-head">
                        <h1 className="t-auth-title">Создать аккаунт</h1>
                        <p className="t-auth-sub">Начни бесплатно — кредитная карта не нужна</p>
                    </div>

                    {error && <div className="t-auth-error">{error}</div>}

                    <form onSubmit={handleRegister} className="t-auth-form">
                        <div className="t-field">
                            <label className="t-field-label">Полное имя</label>
                            <div className="t-field-wrap">
                                <User size={16} className="t-field-icon" />
                                <input type="text" value={name} onChange={e => setName(e.target.value)}
                                    className="t-field-input" placeholder="Алибек Джаксыбеков" required />
                            </div>
                        </div>
                        ...
                        <div className="t-field">
                            ...
                            <div className="t-field-wrap">
                                ...
                                <input type={showPassword ? 'text' : 'password'} value={password}
                                    ...
                                    className="t-field-input" placeholder="Минимум 8 символов" required />
                        ...
                        <label className="t-check-label">
                            <div className="t-check-box" onClick={() => setAgreeTerms(!agreeTerms)} style={agreeTerms ? { background: 'var(--violet)', borderColor: 'var(--violet)' } : {}}>
                                {agreeTerms && <Check size={11} strokeWidth={3} />}
                            </div>
                            <span>Я согласен с <a href="#" className="t-auth-link">Условиями</a> и <a href="#" className="t-auth-link">Политикой</a></span>
                        </label>
                        ...
                        <button type="submit" disabled={isLoading} className="t-btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: 8 }}>
                            {isLoading
                                ...
                                : <><span>Создать аккаунт</span><ArrowRight size={18} /></>
                            }
                        </button>
                    </form>

                    <div className="t-auth-divider"><span>или зарегистрироваться через</span></div>
                    ...
                    <p className="t-auth-switch">
                        Уже есть аккаунт? <Link to="/login" className="t-auth-link">Войти</Link>
                    </p>
                </div>
            </div>
        </div>
